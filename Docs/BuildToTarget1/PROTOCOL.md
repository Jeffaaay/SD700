# BuildToTarget1 protocol / capture contract

Identity: schema0xF10A, build0x4653010C, live profile6/unit2. The existing station1,115200/8N1, FC03/04/05/06 framing, CRC, exact response length and100 ms transaction timeout remain. Every read is split into<=11-register chunks; no CRC/length/error relaxation or START retry.

- FC04 capability0x100:50 config words,284 diagnostic words.
...
- FC03 profile0x180:33 floats, profile6, contact10, target range1..3000, all normal elapsed session/build/capture fields0. Old peak/continuous plan-derived fields must be0. The historical Assist1/2/4 fields are retained but have no Assist admission in this candidate.
...
- FC03 **immutable Build profile0x600:52 words (26 u32)**. High word first; field order and fixed values below. No write range exists. FNV-1a over each u32's little-endian bytes gives digest471320274. Both complete values and diagnostic digest are checked before START.
...
|10|pulse_base_ms|10|
|11|pulse_hard_max_ms|10|
|12|pulse_hard_min_ms|2|
...
Frozen FC04 diagnostics start0x200, end0x31B. Existing86 u32 fields are followed by26 Build u32 fields, then existing27 floats and3 Build floats, exactly as `tools/force_servo_data.py --build-to-target-schema` and the C serializer specify. New fields cover:
