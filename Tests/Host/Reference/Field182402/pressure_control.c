#include "./BSP/PRESSURE_CONTROL/pressure_control.h"
#include "ir2104_driver.h"
...
/* 全局系统实例 */
PressureControlSystem_t g_pressure_sys = {0};
...
/* ---- 自适应助推参数: 脉冲/粗调推不动时自动加大电压，推动后立即复原 ---- */
...
/* PID tuned parameters (unified across Init and SetTarget) */
...
static void PressureControl_HoldMode(PressureControlSystem_t *sys);
static void PressureControl_StopMode(PressureControlSystem_t *sys);
static void PID_Init(PID_Controller_t *pid, float kp, float ki, float kd, float max, float min);
...
static bool     s_hold_motor_disabled = false;
static TimerHandle_t s_pulse_timer = NULL;

static void PressureControl_PulseTimerCallback(TimerHandle_t timer)
{
    PressureControlSystem_t *sys = (PressureControlSystem_t *)pvTimerGetTimerID(timer);
    if (sys != NULL && sys->control.pulse_active) {
        IR2104_Brake();
        sys->control.pulse_active = false;
        sys->control.pulse_end_time = HAL_GetTick();
    }
}

static void PressureControl_StartPulseTimer(PressureControlSystem_t *sys)
{
    /* 定时器队列若满，立即制动，不能让设定脉宽被拉长。 */
    if (s_pulse_timer == NULL ||
        xTimerChangePeriod(s_pulse_timer, pdMS_TO_TICKS(s_cur_pulse_duration), 0) != pdPASS) {
        IR2104_Brake();
        sys->control.pulse_active = false;
        sys->control.pulse_end_time = HAL_GetTick();
    }
...
{
    if (s_hold_motor_disabled) {
        IR2104_Enable();
        s_hold_motor_disabled = false;
    }
}

/* 重置自适应助推状态 (初始化/设定新目标时调用) */
...
/* 远段追求有效压力增量，近段保守调节；只依据上一脉冲方向的真实增量。 */
...
/* 目标前40N和正向精调：只依据上一短脉冲后的新反馈，目标每次增加1-2N。 */
                                              ...
                                              float *voltage, uint32_t *duration)
{
    ...
    if (s_precision_ref_pressure >= 0.0f) {
        if (RS485_2_last_receive_time == s_precision_fb_stamp) {
        ...
        if (rise < 1.0f) {
            ...
            if (rise > PRECISION_PAUSE_RISE_N ||
                ...
                s_near_wait = true;
                s_near_pause_start = HAL_GetTick();
                s_precision_ref_pressure = -1.0f;
                IR2104_Brake();
    ...
    *duration = base_duration + s_precision_extra_ms;
    ...
    s_precision_fb_stamp = RS485_2_last_receive_time;
    return true;
}

/* 反向卸压独立助推：压不动时每两帧加0.5V，已有小幅回落时保持，回落过快时收小。 */
...
void PressureControl_Init(PressureControlSystem_t *sys)
{
    ...
    /* 初始化压力控制器 */
    memset(&sys->control, 0, sizeof(PressureControl_t));
    
    /* 基本参数 */
    sys->control.max_pressure = MAX_PRESSURE_N;
    sys->control.control_interval_ms = 10; /* 状态判断保持原节奏；脉冲由定时器结束 */
    sys->control.last_control_time = HAL_GetTick();
    sys->control.mode = PRESSURE_MODE_STOP;
    sys->control.last_mode = PRESSURE_MODE_STOP;
    
    /* 初始化PID控制器 (使用中的参数) */
    PID_Init(&sys->control.pid, 
             PID_KP_DEFAULT,    /* Kp */
             PID_KI_DEFAULT,    /* Ki */
             PID_KD_DEFAULT,    /* Kd */
             MAX_V,             /* max output voltage */
             MIN_V);            /* min output voltage */
    
    /* 初始化脉冲控制参数 */
    sys->control.pulse_duration_ms = PULSE_DURATION_MS;
    sys->control.pulse_active = false;
    sys->control.pulse_voltage = 1.5f;        /* 脉冲电压1.5V (降低默认值减少冲击) */
    sys->control.pulse_end_time = 0;
    sys->control.pulse_cooldown_ms = PULSE_COOLDOWN_MS;  /* 30ms冷却时间 */
    
    /* 机械参数 (需要根据实际机械结构设置) */
    sys->control.spring_coefficient = 100.0f;  /* 弹性系数 (N/mm) - 示例值 */
    sys->control.screw_pitch = 2.0f;           /* 螺距2mm - 示例值 */
    
    /* 初始化系统状态 */
    sys->system_ready = true;
    sys->pressure_stable = false;
    sys->control_cycles = 0;
    sys->max_error = 0.0f;
    sys->avg_error = 0.0f;
    
    /* 初始化电机驱动 */
    PressureControl_ResetAdaptive();
    IR2104_Init();
    if (s_pulse_timer == NULL) {
        s_pulse_timer = xTimerCreate("PrPulse", pdMS_TO_TICKS(PULSE_DURATION_MS),
                                     pdFALSE, sys, PressureControl_PulseTimerCallback);
    }
    if (s_pulse_timer == NULL) {
        IR2104_Disable();
        sys->system_ready = false; /* 没有定时器时禁止无法限时的自动脉冲 */
        return;
    }
    vTimerSetTimerID(s_pulse_timer, sys);
    PressureControl_Enable(sys);
}

/* 设置目标压力 */
void PressureControl_SetTarget(PressureControlSystem_t *sys, float target_pressure)
{
    if (sys == NULL || !sys->system_ready) return;
    
    /* 压力限制 */
    if (target_pressure > sys->control.max_pressure) {
        target_pressure = sys->control.max_pressure;
    } else if (target_pressure < 0) {
        target_pressure = 0.0f;
    }
    
    /* 设置目标压力 */
    sys->control.target_pressure = target_pressure;
    PressureControl_EnsureMotorEnabled();
    
    /* 重置控制状态 */
    if (s_pulse_timer != NULL) (void)xTimerStop(s_pulse_timer, 0);
    IR2104_Brake();
    sys->control.mode = PRESSURE_MODE_STOP;
    sys->control.pulse_active = false;
    sys->pressure_stable = false;
...
void PressureControl_SetMaxPressure(PressureControlSystem_t *sys, float max_pressure)
{
    ...
    sys->control.max_pressure = max_pressure;
    
    /* 如果当前目标压力超过新限制，则调整目标压力 */
    if (sys->control.target_pressure > max_pressure) {
        sys->control.target_pressure = max_pressure;
    }
}

/* 压力控制主循环 (需要在20ms定时器中断中调用) */
void PressureControl_Update(PressureControlSystem_t *sys)
{
    ...
    if (sys->control.emergency_stop) {
        IR2104_EmergencyStop();
        sys->control.mode = PRESSURE_MODE_EMERGENCY;
        return;
    }

    /* 压力反馈超时保护: 传感器断线/停发时禁止继续输出，防止盲压导致过冲 */
    if (HAL_GetTick() - RS485_2_last_receive_time > PRESSURE_FB_TIMEOUT_MS) {
        IR2104_SetVoltage(0.0f);
        sys->control.pulse_active = false;
        return;
    }
    ...
    if (sys->control.pulse_active &&
        current_time - sys->control.pulse_start_time >= s_cur_pulse_duration) {
        IR2104_Brake();
        sys->control.pulse_active = false;
        sys->control.pulse_end_time = current_time;
    }
    /* 模式/反馈判断仍按约10ms执行。 */
    ...
    if (RS485_2_last_receive_time != s_glitch_feedback_stamp) {
        s_glitch_feedback_stamp = RS485_2_last_receive_time;
    ...
    if (RS485_2_last_receive_time != s_near_feedback_stamp) {
        s_near_feedback_stamp = RS485_2_last_receive_time;
        if (sys->control.pulse_active &&
    ...
    if (s_near_wait) {
        if (HAL_GetTick() - s_near_pause_start < NEAR_SETTLE_WAIT_MS) {
            IR2104_Brake();
            return;
        }
        s_near_wait = false;
    ...
    if (!s_has_contacted && sys->control.current_pressure < 10.0f &&
        ...
        if (sys->control.last_mode != PRESSURE_MODE_HOLD) {
            s_hold_enter_time = HAL_GetTick();
            s_hold_feedback_stamp = RS485_2_last_receive_time;
            s_hold_stable_frames = 0;
            s_hold_last_pressure = -1.0f;
        }
    } 
...
{
    IR2104_SetVoltage(0.0f);
    IR2104_SetMode(IR2104_MODE_STOP);
}

/* 只有首次未接触的接近段连续输出5V；首次达到10N后不再重进。 */
static void PressureControl_CoarseMode(PressureControlSystem_t *sys)
{
    if (sys == NULL) return;
    if (!s_has_contacted && sys->control.current_pressure < 10.0f &&
        sys->control.pressure_error > PRESSURE_ERROR_MICRO_HIGH) {
        IR2104_SetVoltage(APPROACH_VOLTAGE);
    } else {
        IR2104_SetVoltage(0.0f);
        sys->control.mode = PRESSURE_MODE_MICRO;
    }
...
static void PressureControl_MicroMode(PressureControlSystem_t *sys)
{
    ...
    if (sys->control.pulse_active) {
        ...
        if (HAL_GetTick() - sys->control.pulse_start_time >= s_cur_pulse_duration) {
            /* 脉冲结束，短接制动保持(抵抗负载反拖，防止脉冲间隙压力回落) */
            IR2104_Brake();
            sys->control.pulse_active = false;
            sys->control.pulse_end_time = HAL_GetTick();  /* 记录脉冲结束时间 */
        }
        return;
    }
    ...
    if (sys->control.pulse_end_time > 0) {
        ...
        bool settled  = (HAL_GetTick() - sys->control.pulse_end_time >= settle_ms);
        bool fresh    = (RS485_2_last_receive_time != s_pulse_fb_snapshot);
        bool timeout  = (HAL_GetTick() - sys->control.pulse_end_time >= PULSE_FRESH_TIMEOUT_MS);
        if (sys->control.target_pressure <= PRECISION_LOW_TARGET_N) {
            if (RS485_2_last_receive_time != s_low_feedback_stamp) {
                s_low_feedback_stamp = RS485_2_last_receive_time;
    ...
    IR2104_Mode_t pulse_mode = IR2104_MODE_STOP;
    ...
    if (sys->control.pressure_error > PRESSURE_ERROR_MICRO_HIGH) {
        ...
        pulse_mode = IR2104_MODE_POSITIVE;
    } 
    else if (sys->control.pressure_error < PRESSURE_ERROR_MICRO_LOW) {
        /* 误差为负，向上脉冲点动 */
        pulse_voltage = -scaled_voltage;
        pulse_mode = IR2104_MODE_NEGATIVE;
    } 
    else {
        /* 误差在微调阈值以内，进入精调 */
        sys->control.mode = PRESSURE_MODE_FINE;
    ...
    s_pulse_fb_snapshot = RS485_2_last_receive_time;
    s_low_feedback_stamp = s_pulse_fb_snapshot;
    s_low_settle_frames = 0U;

    sys->control.pulse_start_time = HAL_GetTick();
    ...
    IR2104_SetMode(pulse_mode);
    if (pulse_voltage >= 0.0f) {
        pulse_voltage = applied_v;
    } else {
        pulse_voltage = -applied_v;
    }
    IR2104_SetVoltage(pulse_voltage);
    PressureControl_StartPulseTimer(sys);
}

/* 精调模式控制 (S4) - 误差比例精细脉冲点动 + 冷却等待 */
static void PressureControl_FineMode(PressureControlSystem_t *sys)
{
    if (sys == NULL) return;
    s_near_forward_active = false;
...
static void PressureControl_HoldMode(PressureControlSystem_t *sys)
{
    /* 一进入目标范围先主动稳压；只有压力反馈确实稳定才断电自锁。 */
    pressure_substate = PRESSURE_HOLD;

    if (!s_hold_motor_disabled) {
        /* 低于目标时施加小预载，达到或超过目标时短接制动。 */
        if (sys->control.pressure_error > 0.0f) {
            IR2104_SetVoltage(HOLD_PRELOAD_VOLTAGE);
        } else {
            IR2104_Brake();
        }

        bool fresh = (RS485_2_last_receive_time != s_hold_feedback_stamp);
        if (fresh) {
            s_hold_feedback_stamp = RS485_2_last_receive_time;
            float pressure = sys->control.current_pressure;
            if (s_hold_stable_frames == 0U || s_hold_last_pressure < 0.0f ||
                fabsf(pressure - s_hold_last_pressure) > HOLD_STABLE_DELTA_N) {
                s_hold_stable_frames = 1U;
                s_hold_enter_time = HAL_GetTick();
                s_hold_window_min = pressure;
                s_hold_window_max = pressure;
            } else {
                if (pressure < s_hold_window_min) s_hold_window_min = pressure;
                if (pressure > s_hold_window_max) s_hold_window_max = pressure;
                if (s_hold_window_max - s_hold_window_min > HOLD_STABLE_SPAN_N) {
                    s_hold_stable_frames = 1U;
                    s_hold_enter_time = HAL_GetTick();
                    s_hold_window_min = pressure;
                    s_hold_window_max = pressure;
                } else if (s_hold_stable_frames < HOLD_STABLE_FRAME_COUNT) {
                    s_hold_stable_frames++;
                }
            }
            s_hold_last_pressure = pressure;
        }

        if (fresh && sys->control.pressure_error <= 0.0f &&
            s_hold_stable_frames >= HOLD_STABLE_FRAME_COUNT &&
            HAL_GetTick() - s_hold_enter_time >= HOLD_MIN_SETTLE_MS) {
            /* 关闭两路 IR2104，使电机无励磁并依靠机械自锁承载。 */
            IR2104_Disable();
            s_hold_motor_disabled = true;
        }
    }
    
    sys->pressure_stable = s_hold_motor_disabled;
    
    /* 监控压力漂移，如果超出范围，重新进入控制 */
    if (fabsf(sys->control.pressure_error) > PRESSURE_ERROR_FINE_HIGH * 2.0f) {
        /* 如果漂移超过精调范围，重新进入控制 */
        sys->control.mode = PRESSURE_MODE_FINE;
        sys->pressure_stable = false;
    }
...
void PressureControl_EmergencyStop(PressureControlSystem_t *sys)
{
    ...
    sys->control.emergency_stop = true;
    sys->control.mode = PRESSURE_MODE_EMERGENCY;
    IR2104_EmergencyStop();
    s_hold_motor_disabled = false;
}
...
{
    if (sys == NULL) return PRESSURE_MODE_STOP;
    return sys->control.mode;
}

float PressureControl_GetCurrentPressure(PressureControlSystem_t *sys)
{
    if (sys == NULL) return 0.0f;
    return sys->control.current_pressure;
}

float PressureControl_GetTargetPressure(PressureControlSystem_t *sys)
{
    if (sys == NULL) return 0.0f;
    return sys->control.target_pressure;
}

float PressureControl_GetPressureError(PressureControlSystem_t *sys)
{
    if (sys == NULL) return 0.0f;
    return sys->control.pressure_error;
}

bool PressureControl_IsStable(PressureControlSystem_t *sys)
{
    if (sys == NULL) return false;
    return sys->pressure_stable;
}

//手动控制上升下降
void PressureControl_ManualControl()
{
    if(current_interface == INTERFACE_PRESSURE)
    {
        switch (pressure_substate)
        {
            case PRESSURE_JOG_UP :if(IR2104_GetTargetVoltage() != -1.0f)IR2104_SetVoltage(-1.0f);break;
            case PRESSURE_JOG_DOWN :if(IR2104_GetTargetVoltage() != 1.0f)IR2104_SetVoltage(1.0f);break;
            case PRESSURE_FAST_UP :if(IR2104_GetTargetVoltage() != -10.0f)IR2104_SetVoltage(-10.0f);break;
            case PRESSURE_FAST_DOWN :if(IR2104_GetTargetVoltage() != 10.0f)IR2104_SetVoltage(10.0f);break;
            case PRESSURE_IDLE :if(IR2104_GetTargetVoltage() != 0.0f)IR2104_SetVoltage(0.0f);break;
            default :break;
        }
        //通过485判断快速升降逻辑
        if(key_state !=KEY_BACK_LONG)
        {
            if(is_fast_updown == FAST_STOP)
            {
                key_state = KEY_NONE;
                is_fast_updown = FAST_NONE;
            }
            else if(is_fast_updown == FAST_UP && HAL_GetTick() - Up_start_time > 1000) key_state = KEY_UP_LONG;
            ...
            else if(is_fast_updown == FAST_DOWN && HAL_GetTick() - Down_start_time > 1000) key_state = KEY_DOWN_LONG;
        }
    }
...
//一键控制上升下降
void PressureControl_OneTouchControl()
{
    if(current_interface == INTERFACE_ONE_TOUCH)
    {
        if(updowm_substate == UPDOWN_UP && IR2104_GetTargetVoltage() != -10.0f)IR2104_SetVoltage(-10.0f);
        else if(updowm_substate == UPDOWN_DOWN && IR2104_GetTargetVoltage() != 10.0f)IR2104_SetVoltage(10.0f);
        else if(updowm_substate == UPDOWN_IDLE && IR2104_GetTargetVoltage() != 0.0f)IR2104_SetVoltage(0.0f);
    }
}
