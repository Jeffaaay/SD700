...
int main(void) {
    for (int mode=0; mode<3; ++mode) {
        ...
        if (mode==1) {
            assert(machine.fault_detail==FAULT_DETAIL_ASSIST_EXCESSIVE_RISE); off(); /* First post-assist frame must stop before a normal update. */
    ...
    puts("POST_ASSIST_REGRESSION_PASS; synthetic ONLY; live assist DISABLED; hardware NOT_RUN");
    return 0;
}
