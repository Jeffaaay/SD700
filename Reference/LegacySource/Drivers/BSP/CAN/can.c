...
uint8_t can_init(uint32_t tsjw, uint32_t tbs2, uint32_t tbs1, uint16_t brp, uint32_t mode)
{
    ...
    HAL_NVIC_SetPriority(USB_LP_CAN1_RX0_IRQn, 7, 0);                  /* 抢占优先级1，子优先级0 */
#endif
  ...
  /*配置CAN过滤器*/
    ...
    sFilterConfig.FilterMaskIdHigh = 0x7FF << 5;              /* 检查标准ID所有位 */
    sFilterConfig.FilterMaskIdLow = 0x0006;                   /* 检查IDE位(bit2)和RTR位(bit1) */
...
{
    uint32_t id = 0;//接收的CAN数据id=0
    can_receive_msg(id, CAN_rxbuf);
}
...
uint8_t can_send_msg(uint32_t id, uint8_t *msg, uint8_t len)
{
    ...
    uint32_t timeout = 0;
    const uint32_t max_timeout = 100; // 超时阈值（根据实际调整）
    ...
    while (HAL_CAN_GetTxMailboxesFreeLevel(&g_canx_handler) != 3) /* 等待发送完成,所有邮箱(有三个邮箱)为空 */
    {
        ...
        {
            HAL_CAN_AbortTxRequest(&g_canx_handler, TxMailbox); // 中止发送请求
            return 2; // 超时错误码
        }
        delay_us(10); // 短延时避免忙等
    }
