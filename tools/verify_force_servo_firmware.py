...
def verify_contract(sym):
    ...
    require(sym['g_force_build_config']==struct.pack('<33I',2,5000,10,100,8000,800,3000,300,4000,7000,
            10,11,3,50,3,400,1000,1000,30,12000,108000,5000,2,1,2,25,
            2,1,200,500,3500,8500,40000000),'Build configuration assertion failed')
...
def verify(objcopy_cross_check=False):
    ...
    result=dict(configuration='PASS',candidate='BuildToTarget2',schema='F10B',build_id='4653010D',physical_output='COMMISSIONING_ARMED_WITH_EXISTING_GUARD',
                ...
                build_profile=dict(approach_base_command=5000,coarse_boost_step=500,coarse_boost_max=3500,
                    ...
                    post_off_settle_ms=30,total_on_reservation_ms=12000,uninterrupted_off_rest_ms=108000,
