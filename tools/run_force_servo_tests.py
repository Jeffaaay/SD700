...
def main():
    ...
    ap.add_argument('--range-fixture',action='store_true',help='SYNTHETIC host-only ceilings 900/200; not a hardware rating')
    args=ap.parse_args();
    ...
    if args.range_fixture: defines+=['FS_PRESS_PROFILE_CEILING=900','FS_RELEASE_PROFILE_CEILING=200']
    results=[]
    for opt in ('-O0','-O2','-Os'):
