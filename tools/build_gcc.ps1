param(
    ...
    [switch]$AutoTarget,
    [switch]$ForceServo,
    [switch]$Target250Continuous2
)

$ErrorActionPreference = "Stop"
if ($Target250Continuous2 -and -not $ForceServo) { throw 'Target250Continuous2 requires -ForceServo' }
if ($ForceServo -and ($AutoTarget -or $MotorMode -ne 'RealBench')) {
    throw 'ForceServo requires RealBench and excludes AutoTarget'
}
if ($ForceServo) {
    if ($Target250Continuous2) { Write-Host 'TARGET250_CONTINUOUS2; POWERED_TEST_READY_NO; RETAINED_COMMAND_CAP_100_MV; PHYSICAL_TEST_NOT_RUN' }
...
if ($MotorMode -eq "RealBench") {
    ...
    if ($ForceServo) {
        if ($Target250Continuous2) { Write-Host 'TARGET250; NEXT_FRESH_SAMPLE_START; CONTINUOUS_ONLY; EXISTING_HW_GUARD_REQUIRED' }
...
if ($Target250Continuous2) { $defines += 'SD700_FORCE_SERVO_COMMISSIONING=1' }
