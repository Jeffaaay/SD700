...
if ($BuildToTarget2) {
    ...
    Write-Host "BUILD_TO_TARGET2_COOLING_ANCHOR_FIX1; BOUNDED_FORWARD_PRELOAD; TARGET_ONLY; PHYSICAL_NOT_RUN"
}
if ($StaticForceRuntimeCharacterization2) {
    ...
    if (-not $BuildToTarget2) { Write-Host 'STATIC_FORCE_RUNTIME_CHARACTERIZATION2; ONE_RUNTIME_PLAN; USER_CONFIRMED_SENSOR_N; PHYSICAL_NOT_RUN' }
}
...
if ($ForceServo) {
    if ($StaticForceRuntimeCharacterization2) { Write-Host 'RUNTIME_CHARACTERIZATION_GUARDED_OUTPUT; PI_AND_TIMING_LOCKED' }
...
if ($BuildToTarget2) { $defines += 'SD700_BUILD_TO_TARGET=1' }
