...
def build_to_target_metrics(rows, metadata):
    ...
    return dict(build_to_target=True,target_reached=bool(reached),
        ...
        interpulse_observed_samples=sum(bool(r.get('interpulse_active')) for r in selected),
        maximum_interpulse_command=max((r.get('interpulse_command',0) for r in selected),default=0),
        maximum_interpulse_spent_ms=max((r.get('interpulse_spent_ms',0) for r in selected),default=0),
        interpulse_time_accounting='FULL_ENABLED_WALL_TIME_WITHIN_SAME_12000_MS_RESERVATION; NO_PWM_DISCOUNT',
