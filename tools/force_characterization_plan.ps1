...
function New-CharacterizationPlan([double]$TargetForceN,[double]$AssistPercent,[double]$ContinuousPercent) {
    ...
    if ($schema.build_to_target -and ($AssistPercent -ne 0 -or $ContinuousPercent -ne 0)) { throw 'BuildToTarget2 exposes Target only; legacy plan output slots must be zero; ZERO START' }
