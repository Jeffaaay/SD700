...
uint8_t sys_stm32_clock_init(uint32_t plln, uint32_t pllm, uint32_t pllp, uint32_t pllq)
{
    ...
    rcc_clk_init_handle.APB1CLKDivider = RCC_HCLK_DIV2;                 /* APB1分频系数为2 */
    rcc_clk_init_handle.APB2CLKDivider = RCC_HCLK_DIV1;                 /* APB2分频系数为1 */

    ret = HAL_RCC_ClockConfig(&rcc_clk_init_handle, FLASH_LATENCY_3);   /* 同时设置FLASH延时周期为3WS，也就是4个CPU周期 */
    if(ret != HAL_OK)
    {
        return 1;                                                       /* 时钟初始化失败 */
    }
