...
/* Source and design rationale: Docs/BuildToTarget1/SOURCE_BASIS.md.
...
#define FORCE_BUILD_FIELDS(X) \
 X(version,1) X(approach_command,5000) X(contact_N,10) \
 ...
 X(pulse_base_ms,10) X(pulse_hard_max_ms,10) X(pulse_hard_min_ms,2) \
 X(taper_margin_N,50) X(fine_margin_N,3) X(fine_min_command,400) X(fine_max_command,1000) X(fine_hard_ms,5) \
...
typedef struct { uint32_t phase,command,hard_ms; } ForceBuildRequest;
bool ForceBuild_Select(float measured,float target,bool contacted,uint32_t boost,ForceBuildRequest *r);
/* Persistent until a full uninterrupted OFF interval qualifies a new epoch.
 * STOP/START and pulse OFF never clear these progress/boost counters. */
typedef struct {
 uint32_t epoch, phase, no_response_ms, accounted_ms, boost, low_count;
 uint64_t post_sequence;
 float progress_anchor, pulse_before;
