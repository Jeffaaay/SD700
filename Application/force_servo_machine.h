...
#define FORCE_BUILD_DIAG_U32(X) \
 ...
 X(brake_active) X(brake_deadline_ms) X(brake_elapsed_ms) X(segment_settle_ms) \
 X(source_precision_level) X(source_precision_extra_ms) X(source_precision_escape) X(source_precision_band) X(source_near_wait)
#define FORCE_BUILD_DIAG_FLOAT(X) X(pulse_force_before) X(pulse_force_after) X(build_progress_anchor) X(requested_equivalent_V) X(mapped_pwm_percent) X(source_filtered)
#else
#define FORCE_BUILD_DIAG_U32(X)
